...
**Lacunas no código:** não há registro explícito de aceite de termos/privacidade.  
**Consentimento/política/termos:** incluir aceite obrigatório de Termos e Política de Privacidade no cadastro.
...
**Consentimento:** coleta de documentos exige aviso claro e consentimento específico.
...
**Riscos:** PDFs ficam em disco e são servidos por rota **estática** `/api/contracts/files` sem autenticação.  
...
5. **PixKey** gravada em descrição de transações.  
6. **Token em localStorage** (risco XSS).  
...
4. **Contrato com operadores** (Stripe, Azure, Google/Gemini).  
...
- [ ] Registro de consentimento (com timestamp, versão do texto e revogação)  
