...
const MEDALS_REPO: Medal[] = [
  ...
  { id: 'cert-1', name: 'Garçom Elite', icon: 'fa-wine-glass', color: 'text-indigo-600', description: 'Certificado pela Hero Academy', isCertified: true }
];
...
const App: React.FC = () => {
  ...
  const handleStartCourse = (course: Course) => {
     ...
     showToast(`Iniciando módulo: ${course.title}...`, "info");
     setTimeout(() => {
         showToast(`Parabéns! Curso "${course.title}" concluído!`, "success");
         const medal = MEDALS_REPO.find(m => m.id === course.badgeId);
         if(medal) {
             setUser(prev => ({...prev, medals: [...prev.medals, medal]}));
         }
     }, 3000);
  };
            ...
            {view === 'academy' && (
              <div className="space-y-6 animate-in fade-in duration-500">
                <header>
                  ...
                  <p className="text-slate-500 text-sm">Aumente seus ganhos com certificados.</p>
                </header>
                <div className="grid gap-4">
                  {COURSES.map(course => (
